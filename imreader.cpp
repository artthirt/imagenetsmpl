...
void Aug::gen(std::mt19937 &gn)
{
	...
	augmentation = true;
	xoff = (float)ImReader::IM_WIDTH * 0.02 * distr(gn);
	yoff = (float)ImReader::IM_HEIGHT * 0.02 * distr(gn);
	contrast = 0.1 * distr(gn);
	kr = 0.95 + 0.1 * distr(gn);
	kg = kr;//0.95 + 0.1 * distr(gn);
	kb = kr;//0.95 + 0.1 * distr(gn);
	zoomx = 0.9 + 0.15 * distr(gn);
	zoomy = 0.9 + 0.15 * distr(gn);
